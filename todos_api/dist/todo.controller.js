...
export const updateTodo = async (req, res) => {
    try {
        ...
        const updatedTodo = await Todo.findOneAndUpdate({ _id: id, userId }, { title, description, completed }, { new: true }).lean();
        if (!updatedTodo) {
...
export const toggleComplete = async (req, res) => {
    try {
        ...
        const todo = await Todo.findOneAndUpdate({ _id: id, userId }, { completed }, { new: true }).lean();
        if (!todo) {
            return res.status(404).json({ error: "🚫 Could not update the todo. It may not exist or belong to this user." });
        }
